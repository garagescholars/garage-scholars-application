...
export interface MercuryPayoutResult {
  transferId: string | null;
  recipientId: string | null;
  status: "processing" | "pending" | "failed" | "approval_needed" | "queued_insufficient_funds";
  method: "mercury_ach" | "manual_cash";
  error?: string;
}

export interface MercuryBalanceInfo {
  availableBalance: number;
  currentBalance: number;
  accountId: string;
  alertLevel: "ok" | "yellow" | "red" | "critical";
}
...
async function findOrCreateRecipient(
  ...
  if (listResp.ok) {
    ...
    const existing = (Array.isArray(recipients) ? recipients : []).find(
      (r: any) =>
 ...
 */
export async function getMercuryBalance(
  apiKey: string,
  accountId: string
): Promise<MercuryBalanceInfo> {
  ...
  let alertLevel: MercuryBalanceInfo["alertLevel"] = "ok";
  if (available < BALANCE_RED) alertLevel = "red";
  else if (available < BALANCE_YELLOW) alertLevel = "yellow";
 ...
 */
async function autoReplenishFromChase(
  apiKey: string,
  accountId: string,
  currentBalance: number
): Promise<{ pulled: boolean; amount: number; error?: string }> {
  ...
  try {
    const result = await db.runTransaction(async (tx) => {
      ...
      tx.set(replenishRef, {
        ...
        pendingPullStartedAt: FieldValue.serverTimestamp(),
        pendingPullAmount: pullAmount,
        pullsThisWeek: newWeekCount,
        weekResetAt: daysSinceReset >= 7 ? FieldValue.serverTimestamp() : (data.weekResetAt || FieldValue.serverTimestamp()),
      }, { merge: true });
    ...
    await replenishRef.update({
      lastSuccessfulPullAt: FieldValue.serverTimestamp(),
    ...
    await replenishRef.update({
      ...
      lastFailedAt: FieldValue.serverTimestamp(),
      lastFailedError: (err as Error).message,
    }).catch(() => {}); // Don't fail on cleanup

    return { pulled: false, amount: 0, error: (err as Error).message };
  }
}
...
export async function checkBalanceAndAlert(
  ...
  let autoPullResult: { pulled: boolean; amount: number; error?: string } | null = null;
  if (balance.availableBalance < BALANCE_YELLOW) {
    try {
      ...
      autoPullResult = { pulled: false, amount: 0, error: (err as Error).message };
    }
  }
 ...
 */
export async function sendMercuryPayout(
  apiKey: string,
  accountId: string,
  recipient: MercuryRecipient,
  amountDollars: number,
  description: string
): Promise<MercuryPayoutResult> {
  if (!apiKey || !accountId) {
