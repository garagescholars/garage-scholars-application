...
export const gsOnConversationCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    await sendEmail(
      ADMIN_EMAILS,
      `New Buyer Inquiry: ${data.buyerName || "Unknown"} — ${data.itemTitle || "Item"}`,
      emailWrapper(`
...
export const gsOnConversationReply = onDocumentCreated(
  ...
  async (event) => {
    ...
    await db.collection("conversations").doc(conversationId).update({
      lastReplyEmailAt: FieldValue.serverTimestamp(),
    });
    ...
    await sendEmail(
      ADMIN_EMAILS,
      `Reply from ${convo.buyerName || "Buyer"}: ${convo.itemTitle || "Item"}`,
      emailWrapper(`
...
export const gsResaleDripNotifications = onSchedule(
  {
    schedule: "0 9 * * *",
  ...
  async () => {
    ...
    for (const doc of inventorySnap.docs) {
      const item = doc.data();
      const dripsSent = (item.dripsSent as string[]) || [];
      ...
      if (ageMs >= THREE_DAYS_MS && !dripsSent.includes("stale_3d")) {
        ...
        await doc.ref.update({
          dripsSent: FieldValue.arrayUnion("stale_3d"),
        });
      }
      ...
      if (ageMs >= SEVEN_DAYS_MS && !dripsSent.includes("price_drop_7d")) {
        ...
        await doc.ref.update({
          dripsSent: FieldValue.arrayUnion("price_drop_7d"),
        });
      }
    }
    ...
    for (const doc of soldSnap.docs) {
      ...
      if (item.paidOut || dripsSent.includes("payout_reminder")) continue;
      ...
      if (ageMs >= THREE_DAYS_MS) {
        ...
        await doc.ref.update({
          dripsSent: FieldValue.arrayUnion("payout_reminder"),
        });
      }
    }
    ...
    await sendEmail(
      ADMIN_EMAILS,
      `Resale Daily Digest — ${dripItems.length} item${dripItems.length !== 1 ? "s" : ""} need attention`,
      emailWrapper(`
