...
exports.gsGenerateConsultMockup = void 0;
...
function buildGarageShadePrompt(doc, wallColor) {
    ...
    if (rack !== "none" && MONKEY_BARS_DESC[rack])
        items.push(MONKEY_BARS_DESC[rack]);
    // Overhead storage
...
function buildGymShadePrompt(doc, wallColor) {
    ...
    if (acc.includes("dumbbell-rack"))
        items.push("a commercial dumbbell rack");
...
exports.gsGenerateConsultMockup = (0, https_1.onCall)({
    ...
    const { consultationId, shade } = request.data;
    if (!consultationId || !shade) {
        throw new https_1.HttpsError("invalid-argument", "consultationId and shade are required");
    }
    if (!["shade1", "shade2", "shade3"].includes(shade)) {
        throw new https_1.HttpsError("invalid-argument", "shade must be shade1, shade2, or shade3");
    }
    ...
    try {
        ...
        const allReady = ["shade1", "shade2", "shade3"].every((s) => updatedDoc.mockups?.[s]?.status === "ready");
        if (allReady)
