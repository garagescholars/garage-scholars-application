...
function normalizeGarageAddons(raw) {
    if (!raw)
        return { shelving: "none", overheadStorage: "none", cabinets: "none", wallOrg: "none", flooringType: "none", flooringColor: null };
    // Old boolean shape
...
function buildSpaceContext(doc) {
    ...
    if (ceiling === "open-joists")
        sizeStr += " with open joist ceilings";
    ...
    if (states.includes("cluttered"))
        parts.push("The space is currently cluttered with items.");
    if (states.includes("cars-parked"))
        parts.push("Cars may be parked in the space.");
...
function buildFloorInstruction(addons, serviceType) {
    const flooringType = addons.flooringType || "none";
    if (flooringType === "none")
        return "Keep existing floor surface.";
...
function buildGarageOrgPrompt(tier, doc) {
    ...
    switch (tier) {
        ...
        case "tier2": {
            ...
            if (addons.wallOrg === "pegboard")
                items.push("a pegboard tool wall");
            else if (addons.wallOrg === "slatwall")
                items.push("a slatwall organization panel");
            break;
        }
        case "tier3": {
            // Premium: cabinets + overhead + wall org + everything
            if (addons.cabinets === "premium-newage")
                items.push("NewAge Bold Series black steel cabinets lining the walls");
            ...
            if (addons.wallOrg === "slatwall")
                items.push("a slatwall organization system");
            else
                items.push("a pegboard tool wall");
            if (addons.shelving !== "none") {
                const count = addons.shelving === "3-units" ? "three" : addons.shelving === "2-units" ? "two" : "one";
    ...
    switch (tier) {
        ...
        case "tier2": {
            ...
            if (addons.bench !== "none")
                items.push(benchMap[addons.bench] || "an adjustable FID bench");
            else
                items.push("an adjustable FID bench");
            if (addons.cableMachine !== "none")
                items.push(cableMap[addons.cableMachine] || "a cable machine in the corner");
            else
        ...
        case "tier3": {
            ...
            if (acc.includes("mirrors"))
                items.push("full-length mirrors on the back wall");
...
function buildPrompt(serviceType, tier, doc) {
...
exports.gsGenerateConsultMockup = (0, https_1.onCall)({
    ...
    const { consultationId, tier } = request.data;
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", `Mockup generation failed: ${err.message}`);
    }
});
