...
exports.gsCreateInvoice = (0, https_1.onCall)({ cors: true, timeoutSeconds: 30, secrets: ["STRIPE_SECRET_KEY"] }, async (request) => {
    ...
    const { customerName, customerEmail, packageKey, amount, description, jobId, packageTier, lineItems, } = request.data;
    ...
    if (!packageKey && !amount && (!lineItems || lineItems.length === 0)) {
        throw new https_1.HttpsError("invalid-argument", "Either packageKey, amount, or lineItems is required.");
    }
    ...
    if (packageKey) {
        const pkg = gs_constants_1.GS_PACKAGES[packageKey];
        if (!pkg)
            throw new https_1.HttpsError("invalid-argument", `Unknown packageKey: ${packageKey}`);
    ...
    if (!resolvedAmount && (!lineItems || lineItems.length === 0)) {
        throw new https_1.HttpsError("invalid-argument", "Could not determine invoice amount.");
    }
    const stripe = getStripe();
    // Find or create Stripe Customer
    let stripeCustomer;
